...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    for i=1:n
        ...
        C = C + eye(kt, kt) * tol * trace(C)*10;					% Regularization of covariance (if K > D)
    ...
    if strcmp(eig_impl, 'JDQR')
       ...
       % [mappedX, eigenvals] = jdqr(M + eps * eye(n), no_dims + 1, tol, options);
         [mappedX, eigenvals] = jdqr(M + (1-abs(sign(M))).* ones(n,n)*eps*100, no_dims + 1, tol, options);
    else
        options.disp = 0;
        options.isreal = 1;
        options.issym = 1;
       [mappedX, eigenvals] = eigs(M + eps * eye(n), no_dims + 1, tol, options);          % only need bottom (no_dims + 1) eigenvectors
  %  [mappedX, eigenvals] = eigs(M + (1-abs(sign(M))).* ones(n,n)*eps*1000000, no_dims + 1, tol, options);
    end
